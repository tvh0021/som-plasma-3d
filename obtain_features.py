...
# os.chdir("/Users/tha/Downloads/Archive/")
...
def plot_scatter(pair, colorset):
    """Scatter plot given a pair of strings corresponding to the names of the dataset and the label associated with this set

    Args:
        pair (tuple): pair of varible names (string)
        colorset (1D numpy array): the labels associated with the data
    """
    fig, ax = plt.subplots(dpi=100)

    scatter = ax.scatter(globals()[pair[0]].flatten(), globals()[pair[1]].flatten(), c=colorset, s=1, cmap="Spectral", alpha=0.5)
...
def plot_hist2d(pair):
    """2D histogram plot given a pair of strings corresponding to the names of the dataset and the labels associated with it
    ...
    hist2d = ax.hist2d(globals()[pair[0]].flatten(), globals()[pair[1]].flatten(), bins=(200,200), cmap="Spectral")
    plt.xlabel(pair[0])
    plt.ylabel(pair[1])

    return plt.plot()

def get_random_sample(number_of_samples, data_array):
    """Randomly sample a subset of datapoints out of a big dataset

    Args:
        number_of_samples (int): number of samples out of the entire set
        data_array (2D numpy array): n x m array where n is the total number of datapoints and m are the different features

    Returns:
        2D numpy array: number_of_sample x m array
    """
    random_indices = sample(range(0,data_array.shape[0]), number_of_samples)
    generated_sample = np.empty((number_of_samples, data_array.shape[1]))

    for j, k in enumerate(random_indices):
        generated_sample[j] = data_array[k, :]
    
    return generated_sample

def get_smaller_domain(data_array, new_width, start_index):
...
def pseudo_convolution(variable, kernel, step_size : int):
